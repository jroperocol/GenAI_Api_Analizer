...
UNSUPPORTED_AUTH_MARKERS = ["oauth", "sigv4", "mtls", "signature"]
...
def validate_api(api: ApiRecord) -> ApiRecord:
    ...
    if not api.endpoint.raw and not api.endpoint.path:
        if not _has_code(errors, "MISSING_ENDPOINT"):
            errors.append(ApiIssue(code="MISSING_ENDPOINT", message="Endpoint is missing", severity=ErrorSeverity.BLOCKING))

    if api.method is None or api.method.value not in SUPPORTED_METHODS:
        if not _has_code(errors, "MISSING_METHOD"):
            errors.append(ApiIssue(code="MISSING_METHOD", message="HTTP method is missing or unsupported", severity=ErrorSeverity.BLOCKING))

    auth_notes = (api.auth.notes or "").lower()
    if api.auth.type == AuthType.UNKNOWN or any(marker in auth_notes for marker in UNSUPPORTED_AUTH_MARKERS):
        api.auth.goflow_supported = False
        if not _has_code(errors, "AUTH_UNSUPPORTED"):
            errors.append(ApiIssue(code="AUTH_UNSUPPORTED", message="Authentication type unsupported for GoFlow V1", severity=ErrorSeverity.BLOCKING))

    if api.auth.type is None:
        errors.append(ApiIssue(code="MISSING_AUTH", message="Authentication details missing", severity=ErrorSeverity.NON_BLOCKING))

    if not api.headers:
        errors.append(ApiIssue(code="MISSING_HEADERS", message="Required headers may be missing", severity=ErrorSeverity.NON_BLOCKING))

    if api.body.required and not api.body.schema and not api.body.example:
        errors.append(ApiIssue(code="MISSING_BODY_SCHEMA", message="Required body has no schema/example", severity=ErrorSeverity.NON_BLOCKING))

    if api.body.required and not api.body.content_type:
        errors.append(ApiIssue(code="MISSING_CONTENT_TYPE", message="Required body content type missing", severity=ErrorSeverity.NON_BLOCKING))
